...
class AngularAutoCorrelationsTwoAxes(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty("Timestep",
                             "1.0",
                             direction=Direction.Input,
                             doc="Time step between two coordinates in the trajectory in femtoseconds")

        self.declareProperty("SpeciesOne",
                             '',
                             direction=Direction.Input,
                             doc="Specify the first species, e.g. H, He, Li...")
        self.declareProperty("SpeciesTwo",
                             '',
                             direction=Direction.Input,
                             doc="Specify the second species, e.g. H, He, Li...")
        self.declareProperty("SpeciesThree",
                             '',
                             direction=Direction.Input,
                             doc="Specify the third species, e.g. H, He, Li...")

        self.declareProperty(WorkspaceProperty('OutputWorkspace', '', direction=Direction.Output),
                             doc="Output workspace name")
        self.declareProperty(WorkspaceProperty('OutputWorkspaceFT', 'FT', direction=Direction.Output),
                             doc="FT Output workspace name")
    ...
    def PyExec(self):
        ...
        for i in range(3):
            if types[i] not in elements:
                raise RuntimeError('Species ' + ['one', 'two', 'three'][i] +
                                   ' not found in the trajectory file. Please try again...')
        ...
        cartesian_configuration = np.array(
            [[np.dot(box_size_tensors[j], np.transpose(configuration_copy[i, j])) for j in range(n_timesteps)]
        ...
        FT_output_ws = CreateWorkspace(OutputWorkspace=FT_output_name,
                                       DataX=xvals,
                                       UnitX="1/fs",
                                       DataY=yvals,
                                       DataE=evals,
