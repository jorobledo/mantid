...
class ConvertWANDSCDtoQ(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            IMDHistoWorkspaceProperty("NormalisationWorkspace",
                                      ...
                                      direction=Direction.Input), "Workspace to use for normalisation")
        self.declareProperty(
    ...
    def PyExec(self):  # noqa C901
        ...
        qlab = np.vstack(
            (np.sin(polar) * np.cos(azim), np.sin(polar) * np.sin(azim), np.cos(polar) - 1)).T * -k  # Kf - Ki(0,0,1)
        ...
        offset = np.array([[dim0_min / dim0_bin_size], [dim1_min / dim1_bin_size], [dim2_min / dim2_bin_size]]) - 0.5
        ...
        if self.getProperty('KeepTemporaryWorkspaces').value:
            ...
            createWS_alg.setProperty(
                "Extents", '{},{},{},{},{},{}'.format(dim0_min, dim0_max, dim1_min, dim1_max, dim2_min, dim2_max))
        ...
        old_settings = np.seterr(divide='ignore',
                                 invalid='ignore')  # Ignore RuntimeWarning: invalid value encountered in true_divide
        ...
        createWS_alg.setProperty("ErrorInput", np.sqrt(output[1:-1, 1:-1, 1:-1].ravel('F')))
        createWS_alg.setProperty("Dimensionality", 3)
        createWS_alg.setProperty("Extents", '{},{},{},{},{},{}'.format(dim0_min, dim0_max, dim1_min, dim1_max, dim2_min,
                                                                       dim2_max))
        createWS_alg.setProperty("NumberOfBins", '{},{},{}'.format(dim0_bins, dim1_bins, dim2_bins))
        createWS_alg.setProperty("Names", names)
        createWS_alg.setProperty("Units", units)
