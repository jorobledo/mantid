...
#------------------------------------------------------------------------------
    ...
    def _setup(self):
        ...
        self._beam = [
            beam_height, 0.5 * beam_width, -0.5 * beam_width, (beam_width / 2), -(beam_width / 2), 0.0, beam_height,
            0.0, beam_height
        ]
...
#------------------------------------------------------------------------------
    ...
    def _sample(self):
        ...
        sample_ws, self._sample_density = self._set_material(
            self._sample_ws_name, self._set_sample_method, self._sample_chemical_formula,
            self._sample_coherent_cross_section, self._sample_incoherent_cross_section,
            self._sample_attenuation_cross_section, self._sample_density_type, self._sample_density,
        ...
        if self._use_can:
            ...
            can_ws, self._can_density = self._set_material(self._can_ws_name, self._set_can_method,
                                                           self._can_chemical_formula, self._can_coherent_cross_section,
...
#------------------------------------------------------------------------------
    ...
    def _acyl(self, theta, amu_scat, amu_tot_i, amu_tot_s):
        ...
        if self._number_can < 2:
            ...
            #
            AAAA, BBBA, Area_A = self._sum_rom(0, 0, A, self._radii[0], self._radii[1], self._ms, theta, amu_scat,
                                               amu_tot_i, amu_tot_s)
            AAAB, BBBB, Area_B = self._sum_rom(0, 0, -A, self._radii[0], self._radii[1], self._ms, theta, amu_scat,
                                               amu_tot_i, amu_tot_s)
            ...
            for i in range(0, self._number_can - 1):
                ...
                if ms < 1:
                    ms = 1
                AAAA, BBBA, Area_A = self._sum_rom(i, 0, A, radius_1, radius_2, ms, theta, amu_scat, amu_tot_i,
                                                   amu_tot_s)
                AAAB, BBBB, Area_B = self._sum_rom(i, 0, -A, radius_1, radius_2, ms, theta, amu_scat, amu_tot_i,
                                                   amu_tot_s)
                Area_s += Area_A + Area_B
                Ass += AAAA + AAAB
                Assc += BBBA + BBBB
            ...
            if ms < 1:
                ms = 1
            AAAA, BBBA, Area_A = self._sum_rom(nan - 1, 1, A, radius_1, radius_2, ms, theta, amu_scat, amu_tot_i,
                                               amu_tot_s)
            AAAB, BBBB, Area_B = self._sum_rom(nan - 1, 1, -A, radius_1, radius_2, ms, theta, amu_scat, amu_tot_i,
                                               amu_tot_s)
