...
class FindSatellitePeaks(DataProcessorAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(PeaksWorkspaceProperty(name="NuclearPeaks", defaultValue="", direction=Direction.Input),
                             doc="Main integer HKL peaks. Q vectors will be calculated relative to these peaks.")

        self.declareProperty(PeaksWorkspaceProperty(name="SatellitePeaks", defaultValue="", direction=Direction.Input),
        ...
        self.declareProperty(PeaksWorkspaceProperty(name="OutputWorkspace", defaultValue="",
    ...
    def PyExec(self):
        ...
        centroid_satellites = CentroidPeaksMD(InputWorkspace=md,
                                              PeaksWorkspace=predicted_satellites,
                                              PeakRadius=peak_radius,
                                              StoreInADS=False)
        satellites_int_spherical = IntegratePeaksMD(InputWorkspace=md,
    ...
    def create_fractional_peaks_workspace(self, qs, nuclear):
        ...
        for q in qs:
            predicted_q = PredictFractionalPeaks(nuclear,
                                                 HOffset=q[0],
                                                 KOffset=q[1],
                                                 LOffset=q[2],
                                                 StoreInADS=False,
                                                 FracPeaks='predicted_q')
            predicted_satellites = CombinePeaksWorkspaces(predicted_satellites,
                                                          predicted_q,
                                                          StoreInADS=False,
                                                          OutputWorkspace='predicted_satellites')
