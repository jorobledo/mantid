...
class IntegratePeaksProfileFitting(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(WorkspaceProperty(name='OutputPeaksWorkspace', defaultValue='',
        ...
        self.declareProperty(WorkspaceProperty(name='PeaksWorkspace', defaultValue='', direction=Direction.Input),
                             doc='PeaksWorkspace with peaks to be integrated.')

        self.declareProperty(FileProperty(name="UBFile",
                                          defaultValue="",
        ...
        self.declareProperty("DQMax",
                             defaultValue=0.15,
                             doc="Largest total side length (in Angstrom) to consider for profile fitting.")
        self.declareProperty("PeakNumber", defaultValue=-1, doc="Which Peak to fit.  Leave negative for all.")

    def initializeStrongPeakSettings(self, strongPeaksParamsFile, peaks_ws, sampleRun, forceCutoff, edgeCutoff,
        ...
        if strongPeaksParamsFile != "":
            ...
            edgeIDX = np.logical_or.reduce(
                np.array([
                    rows < edgeCutoff, rows > numDetRows - edgeCutoff, cols < edgeCutoff, cols > numDetCols - edgeCutoff
                ]))
    ...
    def PyExec(self):
        ...
        keys = [
            'peakNumber', 'Alpha', 'Beta', 'R', 'T0', 'bgBVG', 'chiSq3d', 'chiSq', 'dQ', 'KConv', 'MuPH', 'MuTH',
            'newQ', 'Scale', 'scale3d', 'SigP', 'SigX', 'SigY', 'Intens3d', 'SigInt3d'
        ]
        ...
        for fitNumber, peakNumber in enumerate(peaksToFit):  #range(peaks_ws.getNumberPeaks()):
            ...
            try:
                ...
                Y3D, goodIDX, pp_lambda, params = BVGFT.get3DPeak(peak,
                                                                  peaks_ws,
                                                                  box,
                                                                  padeCoefficients,
                                                                  qMask,
