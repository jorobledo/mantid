...
class LoadLogPropertyTable(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(
            StringArrayProperty("LogNames",
                                direction=Direction.Input), "The comma seperated list of properties to include. \n" +
    ...
    def getGeneralLogValue(self, ws, name, begin):
        ...
        try:
            ...
            for tt in v.times:
                times2.append(
                    (datetime.datetime(*(time.strptime(str(tt), "%Y-%m-%dT%H:%M:%S")[0:6])) - begin).total_seconds())
    ...
    def PyExec(self):
        ...
        for loopRunNum in range(firstRunNum, lastRunNum + 1):
            ...
            begin = datetime.datetime(*(time.strptime(ws.getRun().getProperty("run_start").value,
                                                      "%Y-%m-%dT%H:%M:%S")[0:6]))  # start of day
            vallist = [loopRunNum]
            for col in collist:
                try:
