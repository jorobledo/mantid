...
class MuonMaxent(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(ITableWorkspaceProperty("InputPhaseTable",
                                                     "",
                                                     direction=Direction.Input,
                                                     optional=PropertyMode.Optional),
                             doc="Phase table (initial guess)")  # from CalMuonDetectorPhases
        self.declareProperty(ITableWorkspaceProperty("InputDeadTimeTable",
                                                     "",
                                                     direction=Direction.Input,
                                                     optional=PropertyMode.Optional),
                             doc="Dead time table (initial)")  # from LoadMuonNexus or blanl=k
        self.declareProperty(ITableWorkspaceProperty("GroupTable",
                                                     "",
                                                     direction=Direction.Input,
                                                     optional=PropertyMode.Optional),
                             doc="Group Table")  # from LoadMuonNexus, none=do all spectra individually
        self.declareProperty(WorkspaceProperty("GroupWorkspace",
                                               "",
                                               direction=Direction.Input,
                                               optional=PropertyMode.Optional),
        ...
        self.declareProperty("OuterIterations",
                             10,
        ...
        self.declareProperty("Factor",
                             1.04,
                             doc="Used to control the value chi-squared converge to",
                             direction=Direction.InOut)
        self.declareProperty(WorkspaceProperty("OutputWorkspace", "", direction=Direction.Output),
                             doc="Output Spectrum (combined) versus field")
        self.declareProperty(ITableWorkspaceProperty("OutputPhaseTable",
                                                     "",
                                                     direction=Direction.Output,
                                                     optional=PropertyMode.Optional),
                             doc="Output phase table (optional)")
        self.declareProperty(ITableWorkspaceProperty("OutputDeadTimeTable",
                                                     "",
                                                     direction=Direction.Output,
                                                     optional=PropertyMode.Optional),
                             doc="Output dead time table (optional)")
        self.declareProperty(WorkspaceProperty("ReconstructedSpectra",
                                               "",
                                               direction=Direction.Output,
                                               optional=PropertyMode.Optional),
                             doc="Reconstructed time spectra (optional)")
        self.declareProperty(WorkspaceProperty("PhaseConvergenceTable",
                                               "",
                                               direction=Direction.Output,
                                               optional=PropertyMode.Optional),
    ...
    def getPhase(self, FLAGS_fixphase, POINTS_ngroups, POINTS_nhists, mylog):
        ...
        if (self.getProperty("InputPhaseTable").isDefault):
            ...
            elif (len(pt) == POINTS_nhists
                  ):  # phase table for ungrouped data. Pick a representative detector for each group (the last one)
                for row in pt:
    ...
    def phaseConvergenceTable(self, POINTS_ngroups, deadDetectors, OuterIter, filePHASE):
        if not self.getProperty("PhaseConvergenceTable").isDefault:
            phaseconvWS = WorkspaceFactory.create("Workspace2D",
    ...
    def PyExec(self):
        ...
        mylog.notice("time zero bin has boundaries {} and {} giving tzero={}".format(
            ws.readX(0)[CHANNELS_itzero],
            ws.readX(0)[CHANNELS_itzero + 1], TZERO_fine))
        t1stgood = self.getProperty("FirstGoodTime").value
        CHANNELS_i1stgood = rg0 + max(int(math.floor((t1stgood - ws.readX(0)[rg0]) / RUNDATA_res + 1.0)),
        ...
        ilast = min(rg0 + int(math.floor((tlast - ws.readX(0)[rg0]) / RUNDATA_res)),
        ...
        (MISSCHANNELS_mm, RUNDATA_fnorm, RUNDATA_hists, MAXPAGE_f, FAC_factor, FAC_facfake, FAC_ratio, DETECT_a,
         DETECT_b, DETECT_c, DETECT_d, DETECT_e, PULSESHAPE_convol, SENSE_taud,
         FASE_phase, SAVETIME_ngo, AMPS_amp, SENSE_phi, OUTSPEC_test, OUTSPEC_guess) = MULTIMAX(
             POINTS_nhists, POINTS_ngroups, POINTS_npts, CHANNELS_itzero, CHANNELS_i1stgood, CHANNELS_itotal,
             RUNDATA_res, RUNDATA_frames, GROUPING_group, DATALL_rdata, FAC_factor, SENSE_taud, MAXPAGE_n, filePHASE,
             PULSES_def, PULSES_npulse, FLAGS_fitdead, FLAGS_fixphase, SAVETIME_i2, OuterIter, InnerIter, mylog, prog,
             phaseconvWS, TZERO_fine, deadDetectors)
        #
        fperchan = 1. / (RUNDATA_res * float(POINTS_npts) * 2.)
        ...
        if (not self.getProperty("ReconstructedSpectra").isDefault):
            ...
            mylog.notice("i1={} i2={} k1={} k2={} len(srcX)={} len(guess)={}".format(
                i1, i2, k1, k2, len(ws.dataX(0)), OUTSPEC_guess.shape[0]))
            recSpec = WorkspaceFactory.create(ws,
                                              NVectors=POINTS_ngroups + len(deadDetectors),
                                              XLength=i2 - i1 + 1,
                                              YLength=i2 - i1)
            offset = 0
