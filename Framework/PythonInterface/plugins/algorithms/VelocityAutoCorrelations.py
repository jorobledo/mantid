    ...
    def PyExec(self):
        ...
        for i in range(n_particles):
            for j in range(n_timesteps - 2):
                ...
                v_temp1 = scaled_coords[i, j + 1] - scaled_coords[i, j] - np.round(scaled_coords[i, j + 1] -
                                                                                   scaled_coords[i, j])
                v_temp2 = scaled_coords[i, j + 2] - scaled_coords[i, j + 1] - np.round(scaled_coords[i, j + 2] -
                                                                                       scaled_coords[i, j + 1])
        ...
        velocities = np.array(
            [[np.dot(box_size_tensors[j + 1], np.transpose(velocities[i, j])) for j in range(n_timesteps - 1)]
             for i in range(n_particles)])
        logger.information(str(time.time() - start_time) + " s")
        ...
        for i in range(n_species):
            for j in range(i, n_species):
                correlations[i,
                             j] = correlations[i, j] * Coh_b[elements[i]] * Coh_b[elements[j]] / correlation_count[i, j]
