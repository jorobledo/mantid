...
class VesuvioAnalysis(PythonAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(
            ...
            validator=StringListValidator(["LoadReduceAnalyse", "ReduceAnalyse", "Analyse"]))
        self.declareProperty(FileProperty("IPFile",
                                          "ip2018.par",
                                          action=FileAction.Load,
                                          direction=Direction.Input,
                                          extensions=["par"]),
                             doc="The instrument parameter file")
        self.declareProperty("NumberOfIterations",
                             2,
                             doc="Number of time the reduction is reiterated.",
        ...
        self.declareProperty(IntArrayProperty("Spectra", [135, 182]),
                             doc="Range of spectra to be analysed (first, last).")
        ...
        self.declareProperty(ITableWorkspaceProperty("ComptonProfile", "", direction=Direction.Input),
                             doc="Table for Compton profiles")
        ...
        self.declareProperty("SubtractResonancesFunction",
                             "",
                             doc="Function for resonance subtraction. Empty means no subtraction.")
        self.declareProperty(
    ...
    def validateInputs(self):
        tableCols = [
            "symbol", "mass(a.u.)", "Intensity lower limit", "Intensity value", "Intensity upper limit",
            "Width lower limit", "Width value", "Width upper limit", "Centre lower limit", "Centre value",
            "Centre upper limit"
        ]
        issues = dict()
        ...
        elif (table.columnCount() != len(tableCols)
              or sorted(cleanNames(tableCols)) != sorted(cleanNames(table.getColumnNames()))):
    ...
    def PyExec(self):
        ...
        if "Reduce" in analysisMode:
            ...
            for iteration in range(number_of_iterations):
                ...
                spectra, widths, intensities, centres = block_fit_ncp(par, first_spectrum, last_spectrum, masses,
                                                                      ws_to_be_fitted, fit_arguments, g_log, IPFile,
                                                                      g_log)
                ...
                if (number_of_iterations - iteration - 1 > 0):
                    ...
                    sample_properties = calculate_sample_properties(masses, mean_widths, mean_intensity_ratios,
                                                                    "GammaBackground", g_log)
                    correct_for_gamma_background(ws_name, first_spectrum, last_spectrum, sample_properties, g_log)  #

                    Scale(InputWorkspace=str(ws_name) + "_gamma_background",
                    ...
                    if transmission_guess < 1.:
                        sample_properties = calculate_sample_properties(masses, mean_widths, mean_intensity_ratios,
                                                                        "MultipleScattering", g_log)
                        correct_for_multiple_scattering(ws_name, first_spectrum, last_spectrum, sample_properties,
                                                        transmission_guess, multiple_scattering_order, number_of_events,
                                                        g_log, masses, mean_intensity_ratios)
                    # Create corrected workspace
                    Minus(LHSWorkspace=ws_name,
                          RHSWorkspace=str(ws_name) + "_gamma_background",
                          OutputWorkspace=ws_name + "_cor")
                    if transmission_guess < 1.:
                        Minus(LHSWorkspace=ws_name + "_cor",
                              RHSWorkspace=str(ws_name) + "_MulScattering",
                              OutputWorkspace=ws_name + "_cor")
                    if subtract_resonances:
                        Minus(LHSWorkspace=ws_name + "_cor",
                              RHSWorkspace=ws_name + "_cor_fit",
                              OutputWorkspace=ws_name + "_cor_residuals")
                        ws = CloneWorkspace(ws_name + "_cor_residuals")
                        for index in range(ws.getNumberHistograms()):
                            Fit(Function=resonance_function,
                        ...
                        Minus(LHSWorkspace=ws_name + "_cor",
                              ...
                              OutputWorkspace=ws_name + "_cor")
                        Minus(LHSWorkspace=ws_name + "_cor_residuals",
                              RHSWorkspace=ws_name + "_fitted_resonances",
                              OutputWorkspace=ws_name + "_cor_residuals")
                else:
                    if fit_hydrogen_in_Y_space:
                        hydrogen_ws = subtract_other_masses(ws_name + "_cor", widths, intensities, centres, spectra,
                                                            masses, IPFile, g_log)
