...
class IndirectILLEnergyTransfer(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty(name='MonitorCutoff',
                             defaultValue=0.5,
        ...
        self.declareProperty(FloatArrayProperty("SampleCoordinates", [0., 0., 0.],
        ...
        self.declareProperty(name='GroupDetectors',
                             defaultValue=True,
    ...
    def validateInputs(self):
        ...
        if not self.getPropertyValue('MapFile'):
            if len(self._psd_int_range) != 2:
                ...
                issues[
                    'ManualPSDIntegrationRange'] = 'Start or end pixel number out is of range [1-128], or has wrong order'
    ...
    def _t0_offset(center_chopper_speed, center_chopper_phase, shifted_chopper_phase, center_psd_delay,
        ...
        """
        return -(shifted_chopper_phase - center_chopper_phase) / center_chopper_speed / 6 + (shifted_psd_delay -
                                                                                             center_psd_delay) * 1E-6
        ...
        if not input_epp:
            ...
            if self._fit_option == 'FitAllPixelGroups':
                GroupDetectors(InputWorkspace=ws,
                               ...
                               GroupingPattern=self._group_pixels(self._group_by))
            ...
            t0_offset = self._t0_offset(center_chopper_speed, center_chopper_phase, shifted_chopper_phase,
                                        center_psd_delay, shifted_psd_delay)
    ...
    def _normalise_to_monitor(self, ws, mon):
        ...
        if self._reduction_type == 'QENS':
            ...
            AddSampleLog(Workspace=ws,
                         LogName="MonitorIntegral",
                         LogType="Number",
                         LogText=str(mtd[int_ws].readY(0)[0]),
                         EnableLogging=False)
            ...
            AddSampleLogMultiple(Workspace=ws,
                                 LogNames=['MonitorLeftPeak', 'MonitorRightPeak'],
