...
from mantid.kernel import (CompositeValidator, Direction, EnabledWhenProperty, FloatArrayBoundedValidator,
                           FloatArrayProperty, IntArrayLengthValidator, IntArrayBoundedValidator, IntArrayProperty,
                           PropertyCriterion, StringArrayLengthValidator, StringArrayProperty, StringListValidator)
...
class PropertyNames(object):
    ...
    PROPETIES_TO_SIZE_MATCH = [
        DB, ANGLE_OPTION, THETA, SUM_TYPE, GROUPING_FRACTION, HIGH_FRG_HALF_WIDTH, HIGH_FRG_HALF_WIDTH_DIRECT,
        HIGH_BKG_OFFSET, HIGH_BKG_OFFSET_DIRECT, HIGH_BKG_WIDTH, HIGH_BKG_WIDTH_DIRECT, LOW_FRG_HALF_WIDTH,
        LOW_FRG_HALF_WIDTH_DIRECT, LOW_BKG_OFFSET, LOW_BKG_OFFSET_DIRECT, LOW_BKG_WIDTH, LOW_BKG_WIDTH_DIRECT,
        START_WS_INDEX, END_WS_INDEX, START_WS_INDEX_DIRECT, END_WS_INDEX_DIRECT, WAVELENGTH_LOWER, WAVELENGTH_UPPER
    ]
...
class ReflectometryILLAutoProcess(DataProcessorAlgorithm):
    ...
    def seeAlso(self):
        ...
        return [
            'ReflectometryILLConvertToQ', 'ReflectometryILLPolarizationCor', 'ReflectometryILLPreprocess',
    ...
    def PyInit(self):
        ...
        #======================== Main Properties ========================
        self.declareProperty(PropertyNames.POLARIZATION_OPTION, 'NonPolarized',
        ...
        is_not_polarized = EnabledWhenProperty(PropertyNames.POLARIZATION_OPTION, PropertyCriterion.IsEqualTo,
                                               'NonPolarized')
        polarized = 'Inputs for polarized measurements'

        self.declareProperty(MultipleFileProperty(PropertyNames.RB, action=FileAction.OptionalLoad, extensions=['nxs']),
                             doc='A list of reflected run numbers/files.')
        self.setPropertySettings(PropertyNames.RB, is_not_polarized)

        self.declareProperty(MultipleFileProperty(PropertyNames.RB00,
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.RB01,
                                                  action=FileAction.OptionalLoad,
                                                  extensions=['nxs']),
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.RB10,
                                                  action=FileAction.OptionalLoad,
                                                  extensions=['nxs']),
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.RB11,
                                                  action=FileAction.OptionalLoad,
                                                  extensions=['nxs']),
        ...
        self.declareProperty(FileProperty(PropertyNames.EFFICIENCY_FILE,
                                          defaultValue='',
                                          action=FileAction.OptionalLoad),
        ...
        self.declareProperty(MultipleFileProperty(PropertyNames.DB, action=FileAction.Load, extensions=['nxs']),
        ...
        self.declareProperty(PropertyNames.BKG_METHOD_DIRECT,
        ...
        self.declareProperty(PropertyNames.BKG_METHOD,
                             defaultValue=BkgMethod.AVERAGE,
                             validator=StringListValidator(
                                 [BkgMethod.AVERAGE, BkgMethod.CONSTANT, BkgMethod.LINEAR, BkgMethod.OFF]),
                             doc='Flat background calculation method for background subtraction.')
        self.copyProperties(
            ...
            [Prop.SUBALG_LOGGING, Prop.CLEANUP, Prop.WATER_REFERENCE, Prop.SLIT_NORM, Prop.FLUX_NORM_METHOD])
        self.declareProperty(PropertyNames.SCALE_FACTOR, defaultValue=1.0, doc='Scale factor.')

        self.declareProperty(PropertyNames.USE_MANUAL_SCALE_FACTORS,
                             defaultValue=False,
        ...
        self.declareProperty(PropertyNames.CACHE_DIRECT_BEAM,
                             defaultValue=False,
        ...
        self.declareProperty(FloatArrayProperty(PropertyNames.THETA, values=[-1.]),
        ...
        self.declareProperty(FloatArrayProperty(PropertyNames.WAVELENGTH_LOWER,
                                                values=[2.],
                                                validator=nonnegativeFloatArray),
                             doc='The lower wavelength bound (Angstrom)')
        self.declareProperty(FloatArrayProperty(PropertyNames.WAVELENGTH_UPPER,
                                                values=[30.],
                                                validator=nonnegativeFloatArray),
        ...
        self.declareProperty(IntArrayProperty(PropertyNames.LOW_FRG_HALF_WIDTH_DIRECT,
                                              values=[2],
        ...
        self.declareProperty(PropertyNames.XMIN_DIRECT,
                             defaultValue=-1.,
        ...
        self.declareProperty(PropertyNames.XMAX_DIRECT,
                             defaultValue=-1.,
        ...
        self.declareProperty(FloatArrayProperty(PropertyNames.XMIN, values=[-1.]),
                             doc='Minimum x value (unit wavelength) used for peak fitting')
        self.setPropertyGroup(PropertyNames.XMIN, preProcessReflected)
        self.declareProperty(FloatArrayProperty(PropertyNames.XMAX, values=[-1.]),
                             doc='Maximum x value (unit wavelength) used for peak fitting')
    ...
    def validateInputs(self):
        ...
        if self.getPropertyValue(
                PropertyNames.POLARIZATION_OPTION) == 'NotPolarized' and not self.getPropertyValue(PropertyNames.RB):
    ...
    def PyExec(self):
        ...
        for angle_index in range(self._dimensionality):
            ...
            if not self.is_polarized():
                ...
                to_convert_to_q = self.process_reflected_beam(reflectedBeamInput, reflectedBeamName, directBeamName,
                                                              angle_index)
            else:
                foreground_names = []
