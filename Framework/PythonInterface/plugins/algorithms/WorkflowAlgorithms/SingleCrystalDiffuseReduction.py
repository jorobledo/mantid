...
from mantid.api import (DataProcessorAlgorithm, mtd, AlgorithmFactory, FileProperty, FileAction, MultipleFileProperty,
                        WorkspaceProperty, PropertyMode, Progress, MatrixWorkspaceProperty, ITableWorkspaceProperty)
from mantid.simpleapi import (LoadIsawUB, MaskDetectors, ConvertUnits, CropWorkspace, LoadInstrument, SetGoniometer,
                              ConvertToMD, MDNorm, MinusMD, Load, DeleteWorkspace, RenameWorkspaces,
...
class SingleCrystalDiffuseReduction(DataProcessorAlgorithm):
    temp_workspace_list = [
        '__run', '__md', '__data', '__norm', '__bkg', '__bkg_md', '__bkg_data', '__bkg_norm', '__normalizedData',
        '__normalizedBackground', 'PreprocessedDetectorsWS'
    ]
    ...
    def PyInit(self):
        # files to reduce
        self.declareProperty(MultipleFileProperty(name="Filename", extensions=["_event.nxs", ".nxs.h5", ".nxs"]),
        ...
        self.declareProperty(
            FileProperty(name="Background",
                         ...
                         extensions=["_event.nxs", ".nxs.h5", ".nxs"]), "Background run")
        ...
        self.declareProperty(
            "ReuseSAFlux", True, "If True then if a previous SolidAngle and Flux has been loaded "
        ...
        # UBMatrix
        self.declareProperty(MultipleFileProperty(name="UBMatrix", extensions=[".mat", ".ub", ".txt"]),
                             doc="Path to an ISAW-style UB matrix text file. See :ref:`LoadIsawUB <algm-LoadIsawUB>`")
        # Goniometer
        self.declareProperty('SetGoniometer', False,
        ...
        if _background:
            ...
            if self.getProperty('KeepTemporaryWorkspaces').value:
                RenameWorkspaces(InputWorkspaces=['__data', '__norm', '__bkg_data', '__bkg_norm'],
                                 WorkspaceNames=[
                                     _outWS_name + '_data', _outWS_name + '_normalization',
    ...
    def load_file_and_apply(self, filename, ws_name, offset):
        ...
        if self._copy_params:
            CopyInstrumentParameters(OutputWorkspace=ws_name,
                                     InputWorkspace=self.getProperty("CopyInstrumentParameters").value)
