...
class INTERReductionTest(systemtesting.MantidSystemTest, ISISReflectometryWorkflowBase):
    ...
    expected_fit_params = {
        'Name': [
            'Theta', 'ScaleFactor', 'AirSLD', 'BulkSLD', 'Roughness', 'BackGround', 'Resolution', 'SLD_Layer0',
            'd_Layer0', 'Rough_Layer0', 'Cost function value'
        ],
        'Value': [
            2.2999999999999998, 1, 0, 0, 0, 7.9488100000033575e-06, 5, 8.2865399998548345e-07, 31.315399998680391, 0,
            0.90910656437440196
        ],
    ...
    def runTest(self):
        self.setupTest()
        stitchTransmissionWorkspaces(self.first_transmission_runs, self.second_transmission_runs,
...
def eventRef(run_number, angle, start=0, stop=0, DB='TRANS'):
    ...
    # Reduce this slice
    ReflectometryISISLoadAndProcess(InputRunList=slice_name,
...
def quickRef(run_numbers=[], trans_workspace_names=[], angles=[]):
    ...
    Stitch1DMany(InputWorkspaces=reduced_runs,
        ...
        for index, (value, expected) in enumerate(zip(values_fitted, reference_values)):
            if abs(value - expected) > tolerance:
                logger.error("For the parameter with index {0}, the value found '{1}' differs from "
                             "reference '{2}' by more than required tolerance '{3}'".format(
                                 index, value, expected, tolerance))
