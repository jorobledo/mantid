...
public:
  ...
  void setUp() override {
    ...
    m_activeFunction = getFunction(getFitFunctionString("dataWorkspace1"), 1);
    ...
    FunctionModelSpectra spectra1 = FunctionModelSpectra("0-3");
    FunctionModelSpectra spectra2 = FunctionModelSpectra("0-3");
    m_fittingData->emplace_back(dataWorkspace1, spectra1);
    m_fittingData->emplace_back(dataWorkspace2, spectra2);
  }
  ...
  void test_that_IndirectFittingModel_instantiates_a_model_with_the_correct_starting_member_variables() {
    ...
    TS_ASSERT_EQUALS(m_model->numberOfWorkspaces(), WorkspaceID{2});
  }

  void test_that_getWorkspace_returns_a_workspace_with_the_correct_number_of_spectra() {
    TS_ASSERT_EQUALS(m_model->getWorkspace()->getNumberHistograms(), 10);
  }
  ...
  void test_that_getGuessWorkspace_returns_a_workspace_with_the_correct_range() {
    m_fittingData->at(0).setStartX(3);
    m_fittingData->at(0).setEndX(8);

    TS_ASSERT_EQUALS(m_model->getGuessWorkspace()->x(0)[0], 3);
    TS_ASSERT_EQUALS(m_model->getGuessWorkspace()->x(0).back(), 8);
  }
  ...
  void test_that_setStartX_does_not_set_the_StartX_when_the_provided_value_is_larger_than_the_EndX() {
    m_fittingData->at(0).setEndX(2.2);
    m_fittingData->at(0).setStartX(8.8);

    TS_ASSERT_EQUALS(m_model->getRange().first, 0.0);
    TS_ASSERT_EQUALS(m_model->getRange().second, 2.2);
  }
  ...
  void test_that_setEndX_does_not_set_the_EndX_when_the_provided_value_is_smaller_than_the_StartX() {
    ...
    TS_ASSERT_EQUALS(m_model->getRange().first, 8.8);
    TS_ASSERT_EQUALS(m_model->getRange().second, 10.0);
  }

  void test_that_getWorkspaceRange_returns_the_defaulted_values_before_a_fit() {

    TS_ASSERT_EQUALS(m_model->getWorkspaceRange().first, 0.0);
    TS_ASSERT_EQUALS(m_model->getWorkspaceRange().second, 10.0);
  }
