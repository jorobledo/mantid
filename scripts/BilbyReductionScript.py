...
class RunBilbyReduction:
    ...
    def setup_save_out_path(self, csv_files_to_reduce_list):
        ...
        reduced_files_path = reduced_files_path_folder + '\\' + self.current_reduction_settings[0][
    ...
    def run_bilby_reduction(self):
        ...
        for current_file in files_to_reduce:
            ...
            if (not external_mode):
                ...
                if wavelength_intervals:
                    wav_delta = float(
                        self.current_reduction_settings[0]["wav_delta"])  # no need to read if the previous is false
