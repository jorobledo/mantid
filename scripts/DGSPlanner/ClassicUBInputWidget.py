...
class ClassicUBInputWidget(QtWidgets.QWidget):
    ...
    def check_state_orientation(self, *dummy_args, **dummy_kwargs):
        ...
        if state == QtGui.QValidator.Acceptable:
            ...
            try:
                uvector = numpy.array(
                    [float(self._editux.text()),
                     float(self._edituy.text()),
                     float(self._edituz.text())])
                vvector = numpy.array(
                    [float(self._editvx.text()),
                     float(self._editvy.text()),
                     float(self._editvz.text())])
    ...
    def validateAll(self):
        ...
        if self.latt_alpha < 5 or self.latt_alpha > 175 or self.latt_beta < 5 or self.latt_beta > 175 or self.latt_gamma < 5 or self.latt_gamma > 175:
            return
        ...
        if numpy.linalg.norm(numpy.cross(uvec, vvec)) < 1e-5:
            return
        self.ol = mantid.geometry.OrientedLattice(self.latt_a, self.latt_b, self.latt_c, self.latt_alpha,
                                                  self.latt_beta, self.latt_gamma)
