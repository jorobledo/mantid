...
#
    ...
    def get_diagnostics_parameters(self):
        ...
        diag_param_list = {
            ...
            'background_test_range': None,
            'instr_name': '',
            'print_diag_results': True,
            'mapmask_ref_ws': None
        }
        result = {}
...
#pylint: disable=bare-except
                        ...
                        cur_val = "Undefined"
                    self.log(
                        "Retrieving or reapplying script property {0} failed. Property value remains: {1}".format(
                            key, cur_val), 'warning')
                    continue
        ...
        if self.instr_name != pInstrument.getName():
            self.log(
                ...
                "*** This only works if both instruments have the same reduction properties!".format(
                    self.instr_name, pInstrument.getName()), 'warning')
...
#pylint: disable=bare-except
            ...
            self.log(
                "Can not retrieve property {0} value from existing reduction parameters. Ignoring this property".format(
                    public_name), 'warning')
            return True
