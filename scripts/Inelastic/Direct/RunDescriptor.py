    ...
    def get_current_run_info(self, sum_runs, ind=None):
        ...
        if ind:
            if not (ind > -1 and ind < len(self._run_numbers)):
                raise RuntimeError("Index {0} is outside of the run list of {1} runs".format(
                    ind, len(self._run_numbers)))
        else:
...
#pylint: disable=too-many-arguments,redefined-outer-name
    ...
    def chop_ws_part(self, origin, tof_range, rebin, chunk_num, n_chunks):
        ...
        if rebin:  # debug and compatibility mode with old reduction
            target = Rebin(origin_name,
                           OutputWorkspace=target_name,
                           Params=[tof_range[0], tof_range[1], tof_range[2]],
        ...
        for monID in mon_list:
            ...
            #pylint: disable=bare-except
                ...
                RunDescriptor._logger(
                    '*** Monitor workspace {0} does not have spectra with ID {1}. Monitor workspace set to None'.format(
                        monws_name, monID), 'warning')
                mon_ws = None
                break
        return mon_ws
#--------------------------------------------------------------------------------------------------------------------
...
#pylint: disable=too-many-branches
    ...
    def apply_calibration(self, loaded_ws, calibration=None, use_ws_calibration=True):
        ...
        if not isinstance(loaded_ws, api.Workspace):
            raise RuntimeError(' Calibration can be applied to a workspace only and got object of type {0}'.format(
                type(loaded_ws)))
        ...
        if use_ws_calibration:
            try:
                ...
                if ws_calibration:
                    ...
                    RunDescriptor._logger(
                        '*** load_data: Calibrating data using workspace defined calibration file: {0}'.format(
                            ws_calibration), 'notice')
        ...
        if isinstance(ws_calibration, str):  # It can be only a file (got it from calibration property)
            RunDescriptor._logger(
                'load_data: Moving detectors to positions specified in cal file {0}'.format(ws_calibration), 'debug')
            ...
            RunDescriptor._logger(
                'load_data: Copying detectors positions from workspace {0}: '.format(ws_calibration.name()), 'debug')
...
#pylint: disable=bare-except
            ...
            try:
                ...
                raise RuntimeError('*** Error: Can not retrieve spectra with ID {0} from source workspace: {1}'.format(
                    spectraID, data_ws.name()))
...
#--------------------------------------------------------------------------------------------------------------------
    ...
    def _load_and_sum_runs(self, inst_name, monitors_with_ws):
        ...
        if sum_ws:
            RunDescriptor._logger("*** Use cached sum of {0} workspaces and adding {1} remaining".format(
                n_already_summed, num_to_sum))
        ...
        for ind, run_num in enumerate(runs_to_sum[load_start:num_to_sum]):
            ...
            if not monitors_with_ws:
                Plus(LHSWorkspace=sum_mon_name,
                     RHSWorkspace=wsp_mon_name,
                     OutputWorkspace=sum_mon_name,
        ...
        AddSampleLog(Workspace=sum_ws_name,
                     LogName=RunDescriptor._sum_log_name,
                     LogText=AddedRunNumbers,
                     LogType='String')
...
#pylint: disable=too-many-arguments
    ...
    def find_file(self, propman, inst_name=None, run_num=None, filePath=None, fileExt=None, **kwargs):
        if self._has_own_value:
            return super(RunDescriptorDependent, self).find_file(propman, inst_name, run_num, filePath, fileExt,
                                                                 **kwargs)
        else:
...
#pylint: disable=too-many-arguments
    ...
    def load_file(self,
        ...
        if self._has_own_value:
            ...
            return self._host.load_file(inst_name, ws_name, run_number, load_mon_with_workspace, filePath, fileExt,
                                        **kwargs)
#pylint: disable=too-many-arguments
    ...
    def load_run(self,
        ...
        if self._has_own_value:
            return super(RunDescriptorDependent, self).load_run(inst_name, calibration, force, mon_load_option,
                                                                use_ws_calibration, filePath, fileExt, **kwargs)
        else:
            return self._host.load_run(inst_name, calibration, force, mon_load_option, use_ws_calibration, filePath,
                                       fileExt, **kwargs)
