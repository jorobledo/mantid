...
def fun_derivative4(x, fun):  # not used at present. Can be used for the H4 polynomial in TOF fitting.
    ...
    for i in range(8, len(fun) - 8):
        ...
        derivative[i] += fun[i + 8] - 32. * fun[i + 7] + 384 * fun[i + 6] - 2016. * fun[i + 5] + 3324. * fun[
            i + 4] + 6240. * fun[i + 3] - 16768 * fun[i + 2] - 4192. * fun[i + 1]
...
def fit_ncp(par, spectrum, masses, data_x, data_y, data_e, fit_arguments, IPFile, g_log):
    ...
    result = optimize.minimize(errfunc,
                               par[:],
...
def calculate_ncp(par, spectrum, masses, data_x, IPFile, g_log):
    ...
    for m in range(len(masses)):  #   [parameter_index + number_of_mass * number_of_parameters_per_mass ]
        ...
        ncp += hei * (joy + FSE) * E0 * E0**(
            -0.92) * mass / delta_Q  # Here -0.92 is a parameter describing the epithermal flux tail.
    return ncp
...
def calculate_resolution(spectrum, data_x, mass, IPFile):
    ...
    dQQ = (1. - (E0 / Ef)**1.5 * L1 / L0 - np.cos(angle / 180. * np.pi) *
           ((E0 / Ef)**0.5 - L1 / L0 * E0 / Ef))**2 * dE1**2
    DQQ_2 = np.abs(Ef / E0 * np.cos(angle / 180. * np.pi) - 1.)
    dQQ += ((2. * E0 * v0 / L0)**2 * dTOF**2 + (2. * E0**1.5 / L0 / Ef**0.5)**2 * dL1**2 +
            (2. * E0 / L0)**2 * dL0**2) * DQQ_2
...
def calculate_mean_widths_and_intensities(masses, widths, intensities, spectra, g_log):
    ...
    mean_widths, widths_std, mean_intensity_ratios = np.zeros((len(masses))), np.zeros((len(masses))), np.zeros(
        (len(masses)))
    for m in range(len(masses)):
...
def correct_for_gamma_background(ws_name, first_spectrum, last_spectrum, sample_properties, g_log):
    ...
    for spec in range(first_spectrum, last_spectrum + 1):
        ...
        tmp_bkg, tmp_cor = sapi.VesuvioCalculateGammaBackground(InputWorkspace=ws_name,
                                                                ComptonFunction=sample_properties,
    ...
    sapi.RenameWorkspace(InputWorkspace="gamma_background_correction",
...
def subtract_other_masses(ws_name, widths, intensities, centres, spectra, masses, IPFile, g_log):
    ...
    for index in range(len(spectra)):  # for each spectrum
        data_x, data_y, data_e = load_workspace(ws_name,
                                                spectra[index])  # get the experimental data after the last correction
        for m in range(len(masses) - 1):  # for all the masses but the first (generally H)
            other_par = (intensities[m + 1, index], widths[m + 1, index], centres[m + 1, index]
...
def convert_to_y_space_and_symmetrise(ws_name, mass):
    ...
    ws = sapi.Rebin(InputWorkspace=ws_name + "_JoY",
...
def correct_for_multiple_scattering(ws_name, first_spectrum, last_spectrum, sample_properties, transmission_guess,
    ...
    dens, trans = sapi.VesuvioThickness(Masses=masses,
                                        Amplitudes=mean_intensity_ratios,
                                        TransmissionGuess=transmission_guess,
                                        Thickness=0.1)
    _TotScattering, _MulScattering = sapi.VesuvioCalculateMS(ws_name,
...
def final_fit(fit_ws_name, constraints, y_range, correct_for_offsets, masses, g_log):
    ...
    g_log.notice("mean kinetic energy: ",
                 sigma_to_energy * ws.cell(0, 1)**2, " +/- ", 2. * sigma_to_energy * ws.cell(0, 2) * ws.cell(0, 1),
                 " meV ")
    if correct_for_offsets:
        sapi.Scale(InputWorkspace=fit_ws_name,
                   Factor=-ws.cell(4, 1),
                   Operation="Add",
                   OutputWorkspace=fit_ws_name + '_cor')
        sapi.Scale(InputWorkspace=fit_ws_name + '_cor',
                   Factor=(2. * np.pi)**(-0.5) / ws.cell(0, 1) / ws.cell(3, 1),
                   Operation="Multiply",
                   OutputWorkspace=fit_ws_name + '_cor')
...
class element:
    def __init__(self, mass, intensity_range, width_range, centre_range):
        ...
        self.intensity_low, self.intensity, self.intensity_high = intensity_range[0], intensity_range[
            1], intensity_range[2]
...
def prepare_fit_arguments(elements, constraints):
    ...
    bounds = ((elements[0].intensity_low, elements[0].intensity_high), (elements[0].width_low, elements[0].width_high),
    ...
    for m in range(len(elements) - 1):
        ...
        bounds += ((elements[m].intensity_low, elements[m].intensity_high),
                   (elements[m].width_low, elements[m].width_high), (elements[m].centre_low, elements[m].centre_high))
    for k in range(len(constraints)):
        ...
        fit_constraints = ({
            'type': constraints[k].type,
            'fun': lambda par: par[lhs_int] - constraints[k].rhs_factor * par[rhs_int]
        })
