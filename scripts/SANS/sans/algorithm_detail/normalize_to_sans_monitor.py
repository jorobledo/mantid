...
def normalize_to_monitor(state_adjustment_normalize_to_monitor, workspace, scale_factor=1.0):
    ...
    workspace = _perform_prompt_peak_correction(workspace, normalize_to_monitor_state)

    # 4. Perform a flat background correction
    workspace = _perform_flat_background_correction(workspace, normalize_to_monitor_state)

    # 5. Convert to wavelength with the specified bin settings.
    workspace = _convert_to_wavelength(workspace, normalize_to_monitor_state)
