...
from sans.user_file.settings_tags import (
    DetectorId, BackId, range_entry, back_single_monitor_entry, single_entry_with_detector, mask_angle_entry, LimitsId,
    simple_range, complex_range, MaskId, mask_block, mask_block_cross, mask_line, range_entry_with_detector, SampleId,
    SetId, set_scales_entry, position_entry, TransId, TubeCalibrationFileId, QResolutionId, FitId, fit_general, MonId,
    monitor_length, monitor_file, GravityId, OtherId, monitor_spectrum, PrintId, det_fit_range, q_rebin_values)
...
class BackParser(UserFileComponentParser):
    ...
    def __init__(self):
        ...
        self._all_mons_pattern = re.compile(start_string + self._all_mons + space_string + float_number + space_string +
                                            float_number + end_string)

        # Single Monitor
        self._mon_id = "M"
        self._single_monitor = "\\s*" + self._mon_id + integer_number + "\\s*"
        self._single_monitor_pattern = re.compile(start_string + self._single_monitor + "(\\s*" + self._times +
        ...
        # Trans
        self._trans = "TRANS"
        self._trans_pattern = re.compile(start_string + self._trans + space_string + float_number + space_string +
                                         float_number)
    ...
    def _extract_single_mon(self, line):
        ...
        time_range = extract_float_range(all_mons_string)
        return {
            BackId.SINGLE_MONITORS: back_single_monitor_entry(monitor=monitor_number,
                                                              start=time_range[0],
                                                              stop=time_range[1])
        }
...
class DetParser(UserFileComponentParser):
    ...
    def __init__(self):
        ...
        self._translation_pattern = re.compile(start_string + self._translation + space_string + float_number +
                                               end_string)
...
class LimitParser(UserFileComponentParser):
    ...
    def __init__(self):
        ...
        self._phi_pattern = re.compile(start_string + self._phi + space_string + float_number + space_string +
                                       float_number + end_string)

        # Event time limits
        self._events_time = "\\s*EVENTSTIME\\s*"
        self._events_time_pattern = re.compile(start_string + self._events_time + space_string + rebin_string_no_comma +
                                               end_string)

        self._events_time_pattern_simple_pattern = re.compile(start_string + self._events_time + space_string +
        ...
        self._qxy_complex_pattern = re.compile(start_string + self._qxy + space_string + self._complex_range +
                                               end_string)
        ...
        self._radius_cut_pattern = re.compile(start_string + self._radius_cut + space_string + float_number +
                                              end_string)
        self._wavelength_cut = "\\s*Q\\s*/\\s*WCUT\\s*"
        self._wavelength_cut_pattern = re.compile(start_string + self._wavelength_cut + space_string + float_number +
                                                  end_string)
    ...
    def _extract_q_limit(self, line):
        ...
        if does_pattern_match(self._q_simple_pattern, line):
            ...
            q_limit_output = [
                complex_output.start, prefix1 * complex_output.step1, complex_output.mid,
    ...
    def _extract_complex_pattern(self, complex_range_input, tag):
        ...
        return {
            tag: complex_range(start=range_with_steps[0],
...
class MaskParser(UserFileComponentParser):
    ...
    def __init__(self):
        ...
        self._single_vertical_strip_pattern = re.compile(start_string + self._detector + self._v + integer_number +
                                                         end_string)
        self._range_vertical_strip_pattern = re.compile(start_string + self._detector + self._v + integer_number +
                                                        self._additional_v + end_string)

        # Horizontal strip Mask
        self._h = "\\s*H\\s*"
        self._additional_h = "(\\s*>" + self._h + integer_number + ")"
        self._single_horizontal_strip_pattern = re.compile(start_string + self._detector + self._h + integer_number +
                                                           end_string)
        self._range_horizontal_strip_pattern = re.compile(start_string + self._detector + self._h + integer_number +
                                                          self._additional_h + end_string)
    ...
    def _extract_block_mask(self, line):
        ...
        if is_true_block:
            ...
            output = {
                MaskId.BLOCK: mask_block(horizontal1=horizontal_part.start,
            ...
            output = {
                MaskId.BLOCK_CROSS: mask_block_cross(horizontal=horizontal_part.entry,
    ...
    def _extract_line_mask(self, line):
        ...
        if length_values == 2:
            ...
            output = {
                MaskId.LINE: mask_line(width=line_values[0], angle=line_values[1], x=line_values[2], y=line_values[3])
            }
        else:
    ...
    def _extract_time_mask(self, line):
        ...
        return {
            key: range_entry_with_detector(start=min_and_max_time[0],
                                           stop=min_and_max_time[1],
                                           detector_type=detector_type)
        }
    ...
    def _extract_vertical_single_strip_mask(self, line):
        ...
        return {
            MaskId.VERTICAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=single_vertical_strip,
                                                                          detector_type=detector_type)
        }
    ...
    def _extract_vertical_range_strip_mask(self, line):
        ...
        return {
            MaskId.VERTICAL_RANGE_STRIP_MASK: range_entry_with_detector(start=range_vertical_strip[0],
    ...
    def _extract_horizontal_single_strip_mask(self, line):
        ...
        return {
            MaskId.HORIZONTAL_SINGLE_STRIP_MASK: single_entry_with_detector(entry=single_horizontal_strip,
                                                                            detector_type=detector_type)
        }
    ...
    def _extract_horizontal_range_strip_mask(self, line):
        ...
        return {
            MaskId.HORIZONTAL_RANGE_STRIP_MASK: range_entry_with_detector(start=range_horizontal_strip[0],
...
class SetParser(UserFileComponentParser):
    ...
    def __init__(self):
        ...
        self._centre_pattern = re.compile(start_string + self._centre + "\\s*(/" + self._lab + space_string + ")?\\s*" +
                                          float_number + space_string + float_number + "\\s*(" + space_string +
                                          float_number + space_string + float_number + ")?\\s*" + end_string)
        self._centre_pattern_HAB = re.compile(start_string + self._centre + "\\s*(/" + self._hab + space_string +
                                              ")?\\s*" + float_number + space_string + float_number + "\\s*(" +
                                              space_string + float_number + space_string + float_number + ")?\\s*" +
                                              end_string)
...
class QResolutionParser(UserFileComponentParser):
    ...
    def __init__(self):
        ...
        self._collimation_length_pattern = re.compile(start_string + self._collimation_length + float_number +
                                                      end_string)
...
class FitParser(UserFileComponentParser):
    ...
    def _extract_general_fit(self, line):
        ...
        return {
            FitId.GENERAL: fit_general(start=wavelength_min,
                                       stop=wavelength_max,
                                       fit_type=fit_type,
                                       data_type=ws_type,
    ...
    def extract_clear():
        ...
        """
        return {
            FitId.GENERAL: fit_general(start=None,
                                       stop=None,
...
class MonParser(UserFileComponentParser):
    ...
    def __init__(self):
        ...
        self._direct_pattern = re.compile(start_string + self._direct + self._optional_detector + self._equal +
                                          self._file_path + end_string)

        # Flat
        self._flat = "\\s*FLAT\\s*"
        self._flat_pattern = re.compile(start_string + self._flat + self._optional_detector + self._equal +
                                        self._file_path + end_string)

        # Flat
        self._hab_file = "\\s*HAB\\s*"
        self._hab_pattern = re.compile(start_string + self._hab_file + self._optional_detector + self._equal +
                                       self._file_path + end_string)
    ...
    def _extract_length(self, line):
        ...
        return {
            MonId.LENGTH: monitor_length(length=length_entries[0], spectrum=length_entries[1], interpolate=interpolate)
        }
...
class PrintParser(UserFileComponentParser):
    ...
    def parse_line(self, line):
        ...
        if setting.upper().startswith(PrintParser.Type):
            ...
            raise RuntimeError("PrintParser: Failed to extract line {} it does not start with {}".format(
                line, PrintParser.Type))
...
class IgnoredParser(object):
    ...
    def __init__(self):
        ...
        # Box mask
        self._mask_pattern = re.compile(start_string + "\\s*MASK\\s*" + integer_number + space_string + integer_number +
                                        space_string + integer_number + space_string + integer_number + end_string)
        ...
        self._back_mon_pattern = re.compile("\\s*BACK\\s*/\\s*M\\s*" + integer_number + r"\." + integer_number +
                                            "\\s*/\\s*TIMES\\s*")
    ...
    def is_ignored(self, line):
        ...
        if (does_pattern_match(self._spy_on_off_pattern, line) or does_pattern_match(self._read_pattern, line)
                ...
                or does_pattern_match(self._back_mon_pattern, line)):  # noqa
            ignore = True
        return ignore
