...
class KpointsData(collections.abc.Sequence):
    ...
    """
    def __init__(self, *, frequencies: np.ndarray, atomic_displacements: np.ndarray, weights: np.ndarray,
                 k_vectors: np.ndarray, unit_cell: np.ndarray) -> None:
        super().__init__()
    ...
    def get_gamma_point_data(self):
        ...
        k_points = {
            "weights": {
                GAMMA_POINT: self._data["weights"][gamma_pkt_index]
            },
            "k_vectors": {
                GAMMA_POINT: self._data["k_vectors"][gamma_pkt_index]
            },
            "frequencies": {
                GAMMA_POINT: self._data["frequencies"][gamma_pkt_index]
            },
            "atomic_displacements": {
                GAMMA_POINT: self._data["atomic_displacements"][gamma_pkt_index]
            },
    ...
    def __getitem__(self, item):  # noqa F811
        if isinstance(item, int):
            return KpointData(self._k_vectors[item], self._weights[item], self._frequencies[item],
