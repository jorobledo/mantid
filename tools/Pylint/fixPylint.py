...
def fixBadIndent(toModify, indent):
    err_string = indent.replace(' Bad indentation. Found ', '').replace('(bad-indentation)',
                                                                        '').split('spaces, expected')
...
def fixParens(fname, errorlist, errlines):
    ...
    for i, error in zip(errlines, errorlist):
        if error.find('(superfluous-parens)') != -1:
            kwd = error.split("'")[1]
            pattern = re.compile(kwd + r'(?P<spaces>\ *)\((?P<file_content>.*)\)')
            match = pattern.search(file_content[i])
            d = match.groupdict()
            newcontent[i] = file_content[i].replace(kwd + d['spaces'] + '(' + d['file_content'] + ')',
                                                    kwd + ' ' + d['file_content'])
...
def generate_choices():
    ...
    ch_help = [
        'simple - fixes the following warning: bad-indentation, missing-final-newline, ' +
