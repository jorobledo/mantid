...
This is a patch release that corrects some significant issues since :ref:`version {version_previous} 
<v{version_previous}>`.
...
def getInfo(number, oauth):
    urls = ['{keyword}/{number:d}'.format(keyword=item, number=number) for item in ['pulls', 'issues']]
    ...
    for url in urls:
        ...
        return dict(number=int(json['number']), url=json['html_url'], title=json['title'].strip())
...
def formatImpact(pullrequests):
    ...
    impact_title = [
        title_format.format(' Issue', ' Impact', ' Solution', ' Side Effect'),
        title_format.format('', '', '', ' Probability'), title_join
    ]

    impact_join = ['-' * length for length in impact_join]
    impact_join = '\n+' + '+'.join(impact_join) + '+'

    impact = [impact_format.format(**pr) + impact_join for pr in pullrequests]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--pulls', nargs='+', type=int, help='specify a list of pull requests and/or issues')
    parser.add_argument('--oauth', help='github oauth token - automatically checks ~/.ssh/github_oauth')
    args = parser.parse_args()

    oauth = getOauth(args.oauth)

    pullrequests = [getInfo(pullrequest, oauth) for pullrequest in args.pulls]
    pullrequests = [pullrequest for pullrequest in pullrequests if pullrequest is not None]

    changes = formatChanges(pullrequests)
    impact = formatImpact(pullrequests)

    config = dict(version=args.release,
